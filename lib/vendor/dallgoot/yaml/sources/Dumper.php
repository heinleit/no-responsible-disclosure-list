...
class Dumper
{
    ...
    public function dump($dataType, int $indent, bool $parentArray = false):string
    {
        if (is_null($dataType)) {
            return '';
            ...
            return $this->handler->dumpCompound($dataType, $indent, $parentArray);
        }
    }
    ...
    public function iteratorToString(\Iterator $iterable,
                                      string $keyMask, string $itemSeparator, int $indent, bool $isArray = false, bool $parentArray = false):string
    {
        $pairs = [];
        ...
        foreach ($iterable as $key => $value) {
            ...
            if ($this->_compactMode) {
				...
				$indent_chars = str_repeat(' ', $indent);
				if($parentArray && $c == 0) {
					$indent_chars = ' ';
				}
				
                $pairs[] = $indent_chars.sprintf($keyMask, $key).$separator.$this->dump($value, $valueIndent, $isArray);
            }
